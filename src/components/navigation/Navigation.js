...
export default function Navigation() {
  ...
  const { token } = useSelector(state => state.userLogin);// récupere le token du reducer
  const { firstName } = useSelector(state => state.userProfile); // récupere le prénom de l'utilisateur

  // lorsque l'utilisateur clique sur SignOut, dispatch l'action logout et le conduit vers l'index
  const logoutHandler = () => {
    dispatch(logout());
    navigate('/');
  };
  ...
  return (
    <nav className="main-nav">
      ...
      <div>
        {!token && ( // retourne le bouton pour se connecter si le token est null
          <Link className="main-nav-item" to="/login">
            {iconUser}             Sign In
          </Link>
        )}
        {token && ( // retourne le prenom et le bouton sign Out si token est vrai
          <>
